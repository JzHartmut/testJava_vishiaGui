...
All Widgets are simple instances from the Java Gral widget classes, first unrelated to graphic implementation. 

All Gral widget classes are derived from link:../../docuSrcJava_vishiaGui/org/vishia/gral/base/GralWidget.html[org.vishia.gral.base.GralWidget]
and/or link:../../docuSrcJava_vishiaGui/org/vishia/gral/base/GralWidgetBase.html[...gral.base.GralWidgetBase].
They contains:
...
[source,Java]
----
include::../../..//testAppl_vishiaGui/java/test_vishiaGral/org/vishia/gral/test/basics/Test_SimpleTextButton_GuiCfg.java[tag=import_class]
----
...
[source,Java]
----
include::../../..//testAppl_vishiaGui/java/test_vishiaGral/org/vishia/gral/test/basics/Test_SimpleTextButton_GuiCfg.java[tag=widgets]
----

Widget references are only necessary if the processing of data need the access to the widgets. 
In this example the [Java]`wdgInputText` is necessary to access, but the [Java]`wdgButton` isn't really. 
...
The widgets can also be accessed by getting the reference by name from the GralMng. 
This is shown in the chapter <<#guiScript>>.  
...
[source,Java]
----
include::../../..//testAppl_vishiaGui/java/test_vishiaGral/org/vishia/gral/test/basics/Test_SimpleTextButton_GuiCfg.java[tag=ctor]
----

image:../../img/Gral/ExampleSimpleTextButton.png[float="right"]
Firstly the ctor of the super class [Java]`GuiCfg` is called, 
That ctor creates the Window instance at Gral Level with the first panel. 
The window's panel is the current one. Hence the [Java]`refPos` refer it and can be used immediately.

The GralWidget instances are defined here all with construction and assignment to the final references if necessary.
If a widget don't need to be referenced, only [java]`new Gral...`widget should be written without assignment. 
The widget is references in its panel and in the [Java]`super.gralMng` instance mediated by the [java]`refPos`.

For the [J]`wdgButton` an action is used which is defined below.
...
[source,Java]
----
include::../../..//testAppl_vishiaGui/java/test_vishiaGral/org/vishia/gral/test/basics/Test_SimpleTextButton_GuiCfg.java[tag=process_actionButton]
----
...
[source,Java]
----
include::../../..//testAppl_vishiaGui/java/test_vishiaGral/org/vishia/gral/test/basics/Test_SimpleTextButton_GuiCfg.java[tag=main]
----

The [J]`main` creates an instance of link:../../docuSrcJava_vishiaGui/org/vishia/gral/base/GuiCallingArgs.html[GuiCallingArgs]
which are necessary for the constructor of 
link:../../docuSrcJava_vishiaGui/org/vishia/gral/cfg/GuiCfg.html[GuiCfg] (the super class). 
Normally this instance can be used to gather command line arguments. But for this example it is not necessary.
If the [J]`sTitle` is set (by command line argument, or direct), it is used as title for the window. 
...
[source,Java]
----
include::../../..//testAppl_vishiaGui/java/test_vishiaGral/org/vishia/gral/test/basics/Test_SimpleTextButton_GuiCfg.java[tag=Actions_class]
----

The actions for buttons are instances of 
link:../../docuSrcJava_vishiaGui/org/vishia/gral/ifc/GralUserAction.html[GralUserAction].
Its operation [J]`exec(...)` should be overridden. But it is proper to call an operation in the main class there.
It is also recommended to wrap all Actions with this here shown [J]`Actions` class.
...
[#gralScript]
== Possibilities to determine the graphic appearance and functions via a script

=== Small example

This is the same example shown in chapter <<#simpleExmpl>> with a script 
